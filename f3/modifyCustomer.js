...
function modifyCustomer(id, newDetails) {
    let customer = customers.find(customer => customer.id === id);
    if (customer) {
        customer.name = newDetails.name || customer.name;
        customer.age = newDetails.age || customer.age;
        customer.address = isAddress(newDetails.address) ? newDetails.address : customer.address;
        customer.newsLetter = newDetails.newsLetter !== undefined ? newDetails.newsLetter : customer.newsLetter;
    }
}
